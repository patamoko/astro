...
## 1. <span style="color: #1f2937;">认识 TypeScript</span>

### 1. <span style="color: pink;">发展史</span>

<span style="color: #4CAF50;">TypeScript 是 JavaScript 的⼀个超集</span>，⽀持 ECMAScript 6 标准（ES6 教程）。 
...
### 1. <span style="color: pink;">JavaScript 是<span style="color: #FF9800;">**动态类型**</span>、<span style="color: #FF9800;">**弱类型**</span>语⾔</span>

<span style="color: #FF9800;">**动态类型**</span>是指在运⾏时才会进⾏类型检查，这种语⾔的类型错误往往会导致运⾏时错误。JavaScript 是⼀⻔ 解释型语⾔，没有编译阶段，所以它是<span style="color: #FF9800;">**动态类型**</span>，以下这段代码在运⾏时才会报错： 

### 2. <span style="color: pink;">TypeScript 是<span style="color: #FF9800;">**静态类型**</span></span>

<span style="color: #FF9800;">**静态类型**</span>是指编译阶段就能确定每个变量的类型，这种语⾔的类型错误往往会导致语法错误。TypeScript 在 运⾏前需要先编译为 JavaScript，⽽在编译阶段就会进⾏类型检查，所以 TypeScript 是<span style="color: #FF9800;">**静态类型**</span> 

### 3. <span style="color: pink;">安装</span>
...
## 2. <span style="color: #1f2937;">使⽤TS</span>
...
## 3. <span style="color: #1f2937;"><span style="color: #FF9800;">**原始数据类型**</span></span>

1.<span style="color: #FF9800;">**原始数据类型**</span>：boolean number string undefined null symbol 
...
<span style="color: #FF9800;">**原始数据类型**</span>的声明⽅式案例 
...
## 4. <span style="color: #1f2937;">其他类型</span>

### 1. <span style="color: pink;">any 类型</span>

:::warning
any 表示的是任意类型，可以任意赋值。⼀个变量设置类型为 `any`，相当于对该变量**关闭了类型检测**。如果声明⼀个变量不设置类型，其实也是 `any` 类型。

> **开发中不建议使⽤ `any`，它会丧失类型保护。**
:::
 
### 2. <span style="color: pink;">unknown 类型</span>

:::info
`unknown` 相对于 `any` 是**安全**的——必须通过类型收窄才能使用。
:::
...
### 3. <span style="color: pink;">never 类型</span>

never 表示永远不会有返回值的类型，⽐如抛出异常，或者死循环，那么该函数永远执⾏不完，不可能有返回 值 

### 4. <span style="color: pink;">值类型</span>
...
## 5. <span style="color: #1f2937;">关于 tsconfig.json⽂件</span>
...
`compilerOptions`（编译器选项）是 `tsconfig.json` 中最常见的配置项：
...
## 6. <span style="color: #1f2937;">数组和元组</span>

### 1. <span style="color: pink;">数组</span>
...
`let arr: Array<类型> = []`  // 这种写法本质上属于<span style="color: #FF9800;">**泛型**</span>
...
### 2. <span style="color: pink;">元组</span>
...
//如果后续通过任何方法向数组添加数据，也可以，后添加的数据是前面每个数据的<span style="color: #FF9800;">**联合类型**</span>
arr2.push("123") 
```

## 7. <span style="color: #1f2937;">对象</span>
...
### 1. <span style="color: pink;">可选属性</span>
...
### 2. <span style="color: pink;">任意属性</span>
...
### 3. <span style="color: pink;">只读属性</span>
...
## 4. <span style="color: #1f2937;">关于内置对象</span>
...
## 8. <span style="color: #1f2937;">函数</span>

### 1. <span style="color: pink;">基本使⽤</span>
...
事实上，上⾯的代码只对等号右侧的匿名函数进⾏了类型定义，⽽等号左边的 fn，是通过赋值操作进 ⾏<span style="color: #FF9800;">**类型推论**</span>⽽推断出来的。如果需要我们⼿动给 fn 添加类型，则应该是这样： 
...
:::tip
**工作中声明函数，更推荐使用 `function` 关键字声明的方式。**
:::
 
### 2. <span style="color: pink;">可选参数</span>
...
### 3. <span style="color: pink;">参数省略</span>
...
### 4. <span style="color: pink;">rest 参数</span>
...
## 9. <span style="color: #1f2937;"><span style="color: #FF9800;">**联合类型**</span></span>

<span style="color: #FF9800;">**联合类型**</span>（Union Types）表示取值可以为多种类型中的⼀种。 

```typescript
//<span style="color: #FF9800;">**联合类型**</span>使用 | 分隔每个类型。
var val:string|number |string[] | ()=>number
val = 12 //ok
val = "hello" //ok 
```

所以我们之前定义存储不同数据数组的时候，也可以使⽤<span style="color: #FF9800;">**联合类型**</span> 

```typescript
const arr: (number | string)[ ] = [1, "string", 2]; 
```

特殊的，如果访问<span style="color: #FF9800;">**联合类型**</span>的属性或⽅法： 

当 TypeScript 不确定⼀个<span style="color: #FF9800;">**联合类型**</span>的变量到底是哪个类型的时候 

我们只能访问此<span style="color: #FF9800;">**联合类型**</span>的所有类型⾥共有的属性或⽅法： 
...
<span style="color: #FF9800;">**联合类型**</span>的写法 
...
## 10. <span style="color: #1f2937;"><span style="color: #FF9800;">**类型别名**</span></span>

在 TypeScript 中，<span style="color: #FF9800;">**类型别名**</span>是给现有类型取⼀个新的名字。它可以⽤于提⾼代码的可读性和可维护性，以及 减少重复的类型定义。 
...
//type 关键字可以为原始值、<span style="color: #FF9800;">**联合类型**</span>、元组以及任何我们定义的类型起⼀个别名 
...
对象数组使⽤<span style="color: #FF9800;">**类型别名**</span>
...
//上面的案例使用<span style="color: #FF9800;">**类型别名**</span>简写
...
结合之前<span style="color: #FF9800;">**联合类型**</span>的写法
...
## 11. <span style="color: #1f2937;"><span style="color: #FF9800;">**类型推论**</span></span>

如果没有明确的指定类型，那么 TypeScript 会依照<span style="color: #FF9800;">**类型推论**</span>（Type Inference）的规则推断出⼀个类型。 
...
函数的参数的<span style="color: #FF9800;">**类型推论**</span> 

```typescript
function fn(x,y){ //x y被推断为any console.log(x+y) } 
```

## 12. <span style="color: #1f2937;">typeof 和 keyof 运算符</span>

## 1. <span style="color: #1f2937;">typeof</span>
...
### 2. <span style="color: pink;">keyof</span>

keyof 运算符接受⼀个对象类型作为参数，返回该对象的所有键名组成的<span style="color: #FF9800;">**联合类型**</span>。 

keyof <span style="color: #FF9800;">**类型别名**</span> 
...
## 13. <span style="color: #1f2937;"><span style="color: #FF9800;">**映射类型**</span></span>

<span style="color: #FF9800;">**映射类型**</span>：基于旧类型创建新类型（对象类型），减少重复，提升开发效率。 

<span style="color: #FF9800;">**映射类型**</span>只能在<span style="color: #FF9800;">**类型别名**</span>中使⽤，不能在接⼝中使⽤。 

in 后⾯跟的是<span style="color: #FF9800;">**联合类型**</span>，也可以是通过 keyof⽣成的<span style="color: #FF9800;">**联合类型**</span> 
...
## 14. <span style="color: #1f2937;">接⼝</span>
...
## 2. <span style="color: #1f2937;">接⼝的继承</span>
...
## 3. <span style="color: #1f2937;">接⼝同名会合并</span>
...
## 4. <span style="color: #1f2937;">接⼝中使⽤<span style="color: #FF9800;">**联合类型**</span></span>
...
## 5. <span style="color: #1f2937;">接⼝也可以⽤于定义数组</span>
...
## 6. <span style="color: #1f2937;">接⼝也可以定义函数</span>
...
## 7. <span style="color: #1f2937;">接⼝和<span style="color: #FF9800;">**类型别名**</span>的区别</span>

interface 与 type 的区别有下⾯几点：
...
## 15. <span style="color: #1f2937;"><span style="color: #FF9800;">**交叉类型**</span></span>

<span style="color: #FF9800;">**交叉类型**</span>（Intersection Types）⽤于组合多个类型，⽣成⼀个包含所有类型特性的新类型。可以 理解为将多个类型合并为⼀个更⼤的类型，新类型拥有所有原始类型的成员。使⽤ & 符号表示交 叉类型。 
...
## 16. <span style="color: #1f2937;">类型断⾔</span>

### 1. <span style="color: pink;">基本⽤法</span>
...
形如 <Foo> 的语法在 tsx 中表示的是一个 ReactNode，在 ts 中除了表示<span style="color: #FF9800;">**类型断言**</span>之外，也可能是表示一个<span style="color: #FF9800;">**泛型**</span> 
故建议大家在使用<span style="color: #FF9800;">**类型断言**</span>时，统一使用 值 as 类型 这样的语法
*/ 
```

## 2. <span style="color: #1f2937;">.将⼀个<span style="color: #FF9800;">**联合类型**</span>断⾔为其中⼀个类型</span>

当 TypeScript 不确定⼀个<span style="color: #FF9800;">**联合类型**</span>的变量到底是哪个类型的时候，我们只能访问此<span style="color: #FF9800;">**联合类型**</span>的所有类型 中共有的属性或⽅法 
...
:::warning
**类型断⾔只能欺骗 TypeScript 编译器**，让它不报错，但**无法避免运行时错误**。使用断言要谨慎！
:::
...
## 3. <span style="color: #1f2937;">将任何⼀个类型断⾔为 any</span>
...
### 4. <span style="color: pink;">将 any 断⾔为任意类型</span>
...
### 5. <span style="color: pink;">将⽗类断⾔为⼦类</span>
...
### 6. <span style="color: pink;">⾮空断⾔</span>
...
### 7. <span style="color: pink;">双重断⾔ (不推荐使⽤)</span>
...
function fn(obj:Girl){
    obj as any as Boy  // ⚠️ 双重断言，不推荐使⽤！
} 
```

## 17. <span style="color: #1f2937;">enum 枚举类型</span>
...
我们常常会有这样的场景，⽐如与后端开发约定订单的状态开始是 0，未结账是 1，运输中是 2，运输完 成是 3，已收货是 4。这样的纯数字会使得代码缺乏可读性。枚举就⽤于这样的场景。<span style="color: #4CAF50;">枚举可以让我们定 义⼀些名字有意义的常量。使⽤枚举可以清晰地表达我们的意图。TypeScript⽀持基于数字枚举和字符 串的枚举。 
...
### 2. <span style="color: pink;">⼿动赋值</span>
...
### 3. <span style="color: pink;">使⽤计算值</span>
...
### 4. <span style="color: pink;">反向映射</span>
...
### 5. <span style="color: pink;">字符串枚举</span>
...
### 6. <span style="color: pink;">异构枚举 （不推荐）</span>
...
## 18. <span style="color: #1f2937;"><span style="color: #FF9800;">**泛型**</span></span> 
<span style="color: #FF9800;">**泛型**</span> Generics）是指在定义函数、接⼝或类的时候，不预先指定具体的类型，⽽在使⽤的时候再指定 类型的⼀种特性 

简单来说<span style="color: #FF9800;">**泛型**</span> 实就是类型参数 

在定义的时候定义形参(类型变量) ，使⽤的时候传⼊实参(实际的类型) 

## 1. <span style="color: #1f2937;">函数中使⽤<span style="color: #FF9800;">**泛型**</span></span> 
```typescript
function identity<T>(arg: T): T {
return arg;
}
identity<Number>(100)
//或者identity(100) 在 TypeScript 中，在调用<span style="color: #FF9800;">**泛型**</span> 数时，如果没有显式地指定<span style="color: #FF9800;">**泛型**</span> 型参数，
//编译器会进行类型推断。根据传入的实际参数类型，编译器可以推断出<span style="color: #FF9800;">**泛型**</span> 型参数的类型，使得函数调用仍然是正确的 
...
## 2. <span style="color: #1f2937;">接⼝中使⽤<span style="color: #FF9800;">**泛型**</span></span> 
```typescript
interface Person<N> {
    name: string;
    age: N;
...
## 3. <span style="color: #1f2937;">类中使⽤<span style="color: #FF9800;">**泛型**</span></span> 
```typescript
class Person<T> {
    name:T,
    age:number
...
<span style="color: #FF9800;">**泛型**</span> 了能使⽤基本类型 string number boolean 等，同时也可以是接⼝，数组等 
...
<span style="color: #FF9800;">**泛型**</span> 可以⽤来定义数组 

`let arr2: Array<number> = [2, 3, 4]`  // Array 是一个内置接口，接受一个 T 类型 
...
## 4. <span style="color: #1f2937;"><span style="color: #FF9800;">**类型别名**</span>中使⽤<span style="color: #FF9800;">**泛型**</span></span> 
```typescript
type C<T>={value:T}
let obj:C<string>={value:"hello"} 
```

### 5. <span style="color: pink;">多个类型参数</span>
...
### 6. <span style="color: pink;">类型参数默认值</span>
...
## 7. <span style="color: #1f2937;">泛型约束</span>

在函数内部使⽤<span style="color: #FF9800;">**泛型**</span> 量的时候，由于事先不知道它是哪种类型，所以不能随意的操作它的属性或⽅法： 
...
## 8. <span style="color: #1f2937;">关于<span style="color: #FF9800;">**泛型**</span> 嵌套</span>

形如 <A<b>> 的嵌套写法表示在 TypeScript 中使⽤<span style="color: #FF9800;">**泛型**</span> ⾏多层嵌套。 

在<span style="color: #FF9800;">**泛型**</span> ，可以使⽤尖括号 < 和 > 将<span style="color: #FF9800;">**泛型**</span> 数括起来。当出现多层嵌套时，例如 <A<b>> ，它表示<span style="color: #FF9800;">**泛型**</span> 型 A 中的⼀个参数 b 是另⼀个<span style="color: #FF9800;">**泛型**</span> 型或带有<span style="color: #FF9800;">**泛型**</span> 数的类型。 

这种嵌套写法主要⽤于更复杂的数据结构和算法设计，它可以在<span style="color: #FF9800;">**泛型**</span> 引⽤其他<span style="color: #FF9800;">**泛型**</span> 型的参数，以实现更灵活和 抽象的泛型约束。 
...
/////本质就是带<span style="color: #FF9800;">**泛型**</span> 类型整体看成一个类型
interface Box<T> {
    item:T
}
...
## 19. <span style="color: #1f2937;">class 类</span>
...
### 1. <span style="color: pink;">关于继承</span>
...
### 2. <span style="color: pink;">访问修饰符</span>

TypeScript 可以使⽤三种访问修饰符（Access Modifiers）：

| 修饰符 | 访问范围 |
|--------|----------|
| `public`（默认） | 任何地方都可以访问 |
| `private` | 只能在**当前类**内部访问 |
| `protected` | 当前类和**子类**中可以访问 | 
...
### 3. <span style="color: pink;">抽象类</span>
...
### 4. <span style="color: pink;">implements 关键字</span>
...
## 20. <span style="color: #1f2937;">模块</span>
...
### 1. <span style="color: pink;">基础的导⼊导出</span>
...
### 2. <span style="color: pink;">类型导出</span>
...
### 3. <span style="color: pink;">类型引⼊</span>
...
## 4. <span style="color: #1f2937;">关于接⼝的默认导出</span>
...
### 5. <span style="color: pink;">重导出</span>
...
## 21. <span style="color: #1f2937;">namespace</span>
...
## 22. <span style="color: #1f2937;">declare 关键字</span>
...
### 2. <span style="color: pink;">⾃动引⼊</span>
...
## 3. <span style="color: #1f2937;">通过 tsconfig.json 配置⾃动加载</span>
...
### 4. <span style="color: pink;">三斜线指令</span>
...
### 2. <span style="color: pink;">第三⽅库</span>
...
## 3. <span style="color: #1f2937;">declare module,declare namespace</span>
...
## 4. <span style="color: #1f2937;">declare 可以 为外部模块添加属性和⽅法时，给出新增部分的 类型描述</span>
...
## 5. <span style="color: #1f2937;">declare module 描述的模块名可以使⽤通配符</span>
...
### 6. <span style="color: pink;">三斜杠命令</span>
...
## 23. <span style="color: #1f2937;">tsconfig.json⽂件详细配置</span>

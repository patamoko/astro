...
### 1. JavaScript 是**动态类型**、**弱类型**语⾔

**动态类型**是指在运⾏时才会进⾏类型检查，这种语⾔的类型错误往往会导致运⾏时错误。JavaScript 是⼀⻔ 解释型语⾔，没有编译阶段，所以它是**动态类型**，以下这段代码在运⾏时才会报错： 

### 2. TypeScript 是**静态类型**

**静态类型**是指编译阶段就能确定每个变量的类型，这种语⾔的类型错误往往会导致语法错误。TypeScript 在 运⾏前需要先编译为 JavaScript，⽽在编译阶段就会进⾏类型检查，所以 TypeScript 是**静态类型** 
...
//如果后续通过任何方法向数组添加数据，也可以，后添加的数据是前面每个数据的**联合类型**
arr2.push("123") 
```
...
特殊的，如果访问**联合类型**的属性或⽅法： 

当 TypeScript 不确定⼀个**联合类型**的变量到底是哪个类型的时候 

我们只能访问此**联合类型**的所有类型⾥共有的属性或⽅法： 
...
在 TypeScript 中，**类型别名**是给现有类型取⼀个新的名字。它可以⽤于提⾼代码的可读性和可维护性，以及 减少重复的类型定义。 
...
如果没有明确的指定类型，那么 TypeScript 会依照**类型推论**（Type Inference）的规则推断出⼀个类型。 
...
keyof 运算符接受⼀个对象类型作为参数，返回该对象的所有键名组成的**联合类型**。 
...
**映射类型**只能在**类型别名**中使⽤，不能在接⼝中使⽤。 

in 后⾯跟的是**联合类型**，也可以是通过 keyof⽣成的**联合类型** 
...
形如 <Foo> 的语法在 tsx 中表示的是一个 ReactNode，在 ts 中除了表示**类型断言**之外，也可能是表示一个**泛型** 
故建议大家在使用**类型断言**时，统一使用 值 as 类型 这样的语法
*/ 
```

## 2. .将⼀个**联合类型**断⾔为其中⼀个类型

当 TypeScript 不确定⼀个**联合类型**的变量到底是哪个类型的时候，我们只能访问此**联合类型**的所有类型 中共有的属性或⽅法 
...
identity<Number>(100)
//或者identity(100) 在 TypeScript 中，在调用**泛型** 数时，如果没有显式地指定**泛型** 型参数，
//编译器会进行类型推断。根据传入的实际参数类型，编译器可以推断出**泛型** 型参数的类型，使得函数调用仍然是正确的 
```
...
**泛型** 了能使⽤基本类型 string number boolean 等，同时也可以是接⼝，数组等 
...
在函数内部使⽤**泛型** 量的时候，由于事先不知道它是哪种类型，所以不能随意的操作它的属性或⽅法： 
...
形如 <A<b>> 的嵌套写法表示在 TypeScript 中使⽤**泛型** ⾏多层嵌套。 

在**泛型** ，可以使⽤尖括号 < 和 > 将**泛型** 数括起来。当出现多层嵌套时，例如 <A<b>> ，它表示**泛型** 型 A 中的⼀个参数 b 是另⼀个**泛型** 型或带有**泛型** 数的类型。 

这种嵌套写法主要⽤于更复杂的数据结构和算法设计，它可以在**泛型** 引⽤其他**泛型** 型的参数，以实现更灵活和 抽象的泛型约束。 
